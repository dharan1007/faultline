...
try{
  ...
  assert.equal(await page.locator('#webmcp').textContent(),'WebMCP ready · 16 tools');
  const connectionText=await page.locator('.connection').textContent();
  assert.match(connectionText,/16 WebMCP tools/i);
  ...
  assert.equal(toolContract.length,16);assert.ok(toolContract.every(t=>t.execute==='function'&&!t.handler));
  ...
  await page.reload({waitUntil:'networkidle'});await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===16);
