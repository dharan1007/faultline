...
try {
  ...
  const state=await page.evaluate(({sinkPort})=>{
    ...
    return window.faultline.loadCase({expectedRevision:current.revision,case:{
      ...
      css:'',
      js:`fetch('http://127.0.0.1:${sinkPort}/fetch-escape').catch(()=>{});document.querySelector('#payload').dataset.afterFetch='yes'`,
  ...
  assert.equal(sinkHits,0,'fetch traffic must never leave the experiment sandbox');
  assert.equal(result.status,'UNRESOLVED','a blocked fetch attempt must not be reported as ordinary oracle evidence');
  assert.equal(result.evidence?.reason,'UNSAFE_NETWORK','fetch rejection must be explicit and machine-readable');
  assert.equal(result.evidence?.capability,'fetch','network evidence must identify the blocked API');

  await page.locator('#preview-run').click();
  await page.waitForTimeout(180);
  assert.equal(sinkHits,0,'explicit preview execution must not issue fetch traffic');
  const summary=await page.locator('#summary').textContent();
  assert.match(summary||'',/UNSAFE_NETWORK · js · fetch/,'blocked preview fetch must be visible to the user');

  console.log('Fetch-network containment PASS: blocked fetch attempts become deterministic unsafe-network evidence in runner and preview.');
} finally {
