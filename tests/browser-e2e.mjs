...
try{
  ...
  await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===17);
  assert.deepEqual(await page.evaluate(()=>Object.keys(window.faultline).sort()),['applySource','autopilot','defineOracle','exportBundle','exportCase','history','importCapture','inspect','loadCase','manifest','pin','probe','reduce','resetCase','restore','revisions','run','units'].sort());
  assert.equal(await page.locator('#webmcp').textContent(),'WebMCP ready · 17 tools');
  const connectionText=await page.locator('.connection').textContent();
  assert.match(connectionText,/17 WebMCP tools/i);
  ...
  assert.equal(toolContract.length,17);assert.ok(toolContract.every(t=>t.execute==='function'&&!t.handler));
  ...
  await page.reload({waitUntil:'networkidle'});await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===17);
  ...
  console.log('Browser gate PASS: Chromium loaded UI, registered 17 spec-valid WebMCP tools with semantic-unit discovery, targeted cancellation, and revision guards on every canonical operation, serialized concurrent sandbox experiments, rejected stale source/oracle mutations, ran oracle, probed, reduced, persisted revisions across reload, restored a pre-reload snapshot, surfaced blocked fetch attempts as unsafe-network evidence, and passed mobile overflow checks.');
