...
const targetJavaScript="document.querySelector('#trigger').addEventListener('click',()=>document.querySelector('#status').setAttribute('data-broken','true'));";
const targetCss='#status{font-weight:700}.noise{display:block}';
const targetHtml='<!doctype html><html><head><title>Captured regression</title><link rel="stylesheet" href="/style.css"></head><body><main><button id="trigger">Trigger</button><p id="status" data-broken="false">ready</p><p class="noise">noise</p></main><script src="/app.js"></script></body></html>';
const targetServerCode=`const http=require('http');const js=${JSON.stringify(targetJavaScript)};const css=${JSON.stringify(targetCss)};const html=${JSON.stringify(targetHtml)};http.createServer((req,res)=>{if(req.url==='/app.js'){res.setHeader('content-type','application/javascript');return res.end(js)}if(req.url==='/style.css'){res.setHeader('content-type','text/css');return res.end(css)}res.setHeader('content-type','text/html; charset=utf-8');res.end(html)}).listen(${targetPort},'127.0.0.1')`;
const targetServer=spawn(process.execPath,['-e',targetServerCode],{stdio:'inherit'});
