...
const sink=http.createServer((req,res)=>{
  if(req.url?.startsWith('/dependency.png'))imageHits++;
...
try {
  ...
  const state=await page.evaluate(({sinkPort})=>{
    ...
    return window.faultline.loadCase({expectedRevision:current.revision,case:{
      html:`<img id="asset" src="http://127.0.0.1:${sinkPort}/dependency.png" alt="dependency">`,
      css:'',
      js:'',
  ...
  assert.equal(imageHits,0,'external image dependencies must never leave the experiment sandbox');
  assert.equal(result.status,'UNRESOLVED','a blocked external image must not be reported as ordinary oracle evidence');
  assert.equal(result.evidence?.reason,'UNSAFE_NETWORK','external image rejection must be explicit and machine-readable');
  ...
  assert.equal(imageHits,0,'explicit preview execution must not request external images');
  const summary=await page.locator('#summary').textContent();
  assert.match(summary||'',/UNSAFE_NETWORK · html · external-image/,'blocked preview image dependency must be visible to the user');

  console.log('HTML image-network containment PASS: blocked external image dependencies become deterministic unsafe-network evidence in runner and preview.');
} finally {
