...
const replaceOnce=(pattern,replacement,label)=>{
  const matches=typeof pattern==='string'?source.split(pattern).length-1:(source.match(new RegExp(pattern.source,pattern.flags.includes('g')?pattern.flags:pattern.flags+'g'))||[]).length;
  if(matches!==1)throw new Error(`${label}: expected exactly one match, got ${matches}`);
  source=source.replace(pattern,replacement);
};

replaceOnce(
  /function executeCase\(c,\{signal\}=\{\}\)\{[\s\S]*?\n\}\nfunction runCase/,
...
replaceOnce(
  ...
  'commitCase preview sync'
);
replaceOnce(
  "  persistMutation(before);\n  render();\n  return inspect();\n}\nfunction resetCase",
  "  persistMutation(before);\n  render();\n  renderPreview();\n  return inspect();\n}\nfunction resetCase",
  'loadCase preview sync'
);
replaceOnce(
...
replaceOnce(
  "function render(){const s=inspect();",
