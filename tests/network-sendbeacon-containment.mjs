...
try {
  ...
  const state=await page.evaluate(({sinkPort})=>{
    ...
    return window.faultline.loadCase({expectedRevision:current.revision,case:{
      ...
      css:'',
      js:`try{navigator.sendBeacon('http://127.0.0.1:${sinkPort}/beacon','escaped')}catch{};document.querySelector('#payload').dataset.afterBeacon='yes'`,
  ...
  assert.equal(sinkHits,0,'sendBeacon requests must never leave the experiment sandbox');
  assert.equal(result.status,'UNRESOLVED','a blocked sendBeacon attempt must not be reported as ordinary oracle evidence');
  assert.equal(result.evidence?.reason,'UNSAFE_NETWORK','sendBeacon rejection must be explicit and machine-readable');
  assert.equal(result.evidence?.capability,'sendbeacon','network evidence must identify the blocked API');

  await page.locator('#preview-run').click();
  await page.waitForTimeout(220);
  assert.equal(sinkHits,0,'explicit preview execution must not issue sendBeacon requests');
  const summary=await page.locator('#summary').textContent();
  assert.match(summary||'',/UNSAFE_NETWORK · js · sendbeacon/,'blocked preview sendBeacon must be visible to the user');

  console.log('sendBeacon-network containment PASS: blocked sendBeacon attempts become deterministic unsafe-network evidence in runner and preview.');
} finally {
