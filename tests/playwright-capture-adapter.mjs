...
const {captureFaultlineCase}=adapter;
assert.equal(typeof adapter.armFaultlineDiagnostics,'function','Playwright capture adapter must expose an armed diagnostics recorder before a failing step');
const {armFaultlineDiagnostics}=adapter;
...
try{
  ...
  // Arm diagnostics before the failing browser step. Evidence emitted before the eventual
  // snapshot must survive into the portable capture rather than being lost.
  const diagnostics=armFaultlineDiagnostics({page});
  await page.evaluate(()=>console.error('pre-snapshot console failure'));
  await page.evaluate(()=>setTimeout(()=>{throw new Error('pre-snapshot page boom')},0));
  ...
  const capture=await captureFaultlineCase({page,oracle,js,provenance:{testTitle:'profile save regression',testFile:'profile.spec.mjs'},diagnostics});
  ...
  assert.ok(capture.diagnostics.consoleErrors.some(message=>message.includes('pre-snapshot console failure')),'console.error evidence emitted before capture must survive');
  assert.ok(capture.diagnostics.pageErrors.some(message=>message.includes('pre-snapshot page boom')),'uncaught pageerror evidence emitted before capture must survive');
  ...
  diagnostics.dispose();
  const beforeDisposeSnapshot=diagnostics.snapshot();
  await page.evaluate(()=>console.error('must-not-be-recorded-after-dispose'));
  await page.waitForTimeout(10);
  assert.deepEqual(diagnostics.snapshot(),beforeDisposeSnapshot,'disposed diagnostics recorder must detach listeners deterministically');
  diagnostics.dispose();

  console.log('Playwright capture adapter PASS: a caller-owned prepared page becomes a bounded portable FAULTLINE capture with live form state and armed pre-snapshot diagnostics preserved.');
}finally{
