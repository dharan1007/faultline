...
try{
  ...
  state=await page.evaluate(({revision,sinkPort})=>window.faultline.loadCase({
    ...
    case:{
      html:`<main id="payload">contained<img src="http://127.0.0.1:${sinkPort}/pixel?secret=faultline" alt=""></main>`,
      css:'',
      js:'',
  ...
  assert.equal(sinkHits,0,'candidate HTML resources must never leave the isolated experiment sandbox');
