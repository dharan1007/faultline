...
try{
  ...
  await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===18);
  assert.deepEqual(await page.evaluate(()=>Object.keys(window.faultline).sort()),['applySource','autopilot','defineOracle','exportCapture','exportCase','history','importCapture','inspect','loadCase','manifest','pin','probe','reduce','resetCase','restore','revisions','run','units'].sort());
  assert.equal(await page.locator('#webmcp').textContent(),'WebMCP ready · 18 tools');
  const connectionText=await page.locator('.connection').textContent();
  assert.match(connectionText,/18 WebMCP tools/i);
  ...
  assert.equal(toolContract.length,18);assert.ok(toolContract.every(t=>t.execute==='function'&&!t.handler));

  const guardedToolNames=['faultline_load_case','faultline_import_capture','faultline_reset_case','faultline_run','faultline_define_oracle','faultline_apply_source','faultline_probe','faultline_reduce','faultline_pin','faultline_restore','faultline_autopilot'];
  ...
  for(const name of ['faultline_run','faultline_import_capture','faultline_probe','faultline_reduce','faultline_autopilot']){
  ...
  const unitsToolContract=await page.evaluate(()=>{const t=window.__webmcpTools.find(tool=>tool.name==='faultline_units');return t&&{properties:Object.keys(t.inputSchema?.properties||{}).sort(),required:[...(t.inputSchema?.required||[])].sort(),additionalProperties:t.inputSchema?.additionalProperties};});
  assert.deepEqual(unitsToolContract,{properties:['targetAxis'],required:['targetAxis'],additionalProperties:false});
  ...
  await page.reload({waitUntil:'networkidle'});await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===18);
  ...
  console.log('Browser gate PASS: Chromium loaded UI, registered 18 spec-valid WebMCP tools including capture import/export with semantic-unit discovery, targeted cancellation, and revision guards on every canonical operation, serialized concurrent sandbox experiments, rejected stale source/oracle mutations, ran oracle, probed, reduced, persisted revisions across reload, restored a pre-reload snapshot, surfaced blocked fetch attempts as unsafe-network evidence, and passed mobile overflow checks.');
