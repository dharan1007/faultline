...
try{
  ...
  assert.deepEqual(await page.evaluate(()=>Object.keys(window.faultline).sort()),['applySource','autopilot','defineOracle','exportCase','history','inspect','loadCapture','loadCase','manifest','pin','probe','reduce','resetCase','restore','revisions','run','units'].sort());
  ...
  assert.match(connectionText,/faultline_load_capture/);
  ...
  const guardedToolNames=['faultline_load_case','faultline_load_capture','faultline_reset_case','faultline_run','faultline_define_oracle','faultline_apply_source','faultline_probe','faultline_reduce','faultline_pin','faultline_restore','faultline_autopilot'];
  ...
  console.log('Browser gate PASS: Chromium loaded UI, registered 17 spec-valid WebMCP tools including Playwright capture ingestion, semantic-unit discovery, targeted cancellation, and revision guards on every canonical operation, serialized concurrent sandbox experiments, rejected stale source/oracle mutations, ran oracle, probed, reduced, persisted revisions across reload, restored a pre-reload snapshot, surfaced blocked fetch attempts as unsafe-network evidence, and passed mobile overflow checks.');
