...
try{
  ...
  assert.deepEqual(await page.evaluate(()=>Object.keys(window.faultline).sort()),['applySource','autopilot','defineOracle','exportCase','history','inspect','loadCase','manifest','pin','probe','reduce','resetCase','restore','run'].sort());
  assert.equal(await page.locator('#webmcp').textContent(),'WebMCP ready · 13 tools');
  const connectionText=await page.locator('.connection').textContent();
  assert.match(connectionText,/thirteen WebMCP tools/i);
  ...
  assert.equal(toolContract.length,13);assert.ok(toolContract.every(t=>t.execute==='function'&&!t.handler));
  ...
  await page.reload({waitUntil:'networkidle'});await page.waitForFunction(()=>window.faultline && window.__webmcpTools?.length===13);
  ...
  console.log('Browser gate PASS: Chromium loaded UI, registered 13 spec-valid WebMCP tools with revision guards on every canonical operation, serialized concurrent sandbox experiments, rejected stale source/oracle mutations, ran oracle, probed, reduced, persisted revisions across reload, restored a pre-reload snapshot, blocked experiment network side effects, and passed mobile overflow checks.');
