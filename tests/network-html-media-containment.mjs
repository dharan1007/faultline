...
const sink=http.createServer((req,res)=>{
  if(req.url?.startsWith('/dependency.mp4'))mediaHits++;
...
try {
  ...
  const embeddedState=await page.evaluate(()=>{
    const current=window.faultline.inspect();
    return window.faultline.loadCase({expectedRevision:current.revision,case:{
      html:'<audio id="media" src="data:audio/wav;base64,UklGRg=="></audio>',
      css:'',
      js:'',
  ...
  assert.equal(embeddedResult.status,'FAIL','self-contained data media must remain executable and preserve ordinary oracle evidence');

  console.log('HTML media-network containment PASS: blocked external media dependencies become deterministic unsafe-network evidence while data media remains runnable.');
} finally {
