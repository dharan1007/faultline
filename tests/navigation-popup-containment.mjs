...
try{
  ...
  const js=`window.open('http://127.0.0.1:${sinkPort}/popup-escape','_blank')`;
  state=await page.evaluate(({revision,js})=>window.faultline.applySource({expectedRevision:revision,targetAxis:'js',source:js}),{revision:state.revision,js});
  ...
  assert.equal(sinkHits,0,'popup navigation must not reach the network');
  assert.equal(result.status,'UNRESOLVED','a browser-blocked popup attempt must not be reported as ordinary PASS/FAIL evidence');
  assert.equal(result.evidence?.reason,'UNSAFE_NAVIGATION');
  assert.equal(result.evidence?.capability,'popup-navigation');
  console.log('Popup-navigation containment PASS: blocked popup attempts are surfaced as deterministic unsafe navigation evidence.');
} finally {
