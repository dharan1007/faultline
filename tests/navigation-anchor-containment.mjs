...
try{
  ...
  const html=`<main id="payload">contained</main><a id="escape" href="http://127.0.0.1:${sinkPort}/anchor-escape">escape</a>`;
  ...
  state=await page.evaluate(({revision})=>window.faultline.applySource({expectedRevision:revision,targetAxis:'js',source:"document.querySelector('#escape').click()"}),{revision:state.revision});
  ...
  assert.equal(sinkHits,0,'script-triggered external anchor navigation must never leave the experiment sandbox');
  assert.equal(result.status,'UNRESOLVED','cases capable of external anchor navigation must be rejected before sandbox execution');
  assert.equal(result.evidence?.reason,'UNSAFE_NAVIGATION','anchor navigation rejection must be explicit and machine-readable');
  console.log('Anchor-navigation containment PASS: script-triggered external links cannot escape the experiment sandbox.');
} finally {
